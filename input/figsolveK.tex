...
\begin{tikzpicture}[scale=1.3]
  ...
  \draw[->](5+0*\X, 5+0*\Y) -- node[above]{$c_3$ $\bm{b_2}$ ~~~~~~~~ $\BB{\bm{[b_2]}}$} (-5+2*\X, 5+0*\Y); % A->C
  \draw[<-](5+0*\X,  1.25+ 0*\Y) -- node[below right]{~~ $\BB{\bm{b_2}}$} (-5+2*\X,  1.25+ 0*\Y); % A<-C
  
  \draw[->](5+1*\X, -1*\Y) -- node[below right, sloped]{$\BP{\bm{[c_3]}}$} (-5+2*\X, 0*\Y); %% B<-C
